...
export function LegalPage({ title, updated, children }: { title: string; updated: string; children: ReactNode }) {
  return <><Header/><main id="main-content" className="mx-auto max-w-3xl px-5 py-16 md:py-24"><div className="mb-10"><Link href="/" className="text-sm text-primary hover:underline">秒速ツール</Link><span className="mx-2 text-muted-foreground">/</span><span className="text-sm text-muted-foreground">{title}</span></div><article className="prose prose-slate max-w-none dark:prose-invert"><h1 className="text-3xl font-bold tracking-tight md:text-4xl">{title}</h1><p className="text-sm text-muted-foreground">最終更新日：{updated}</p>{children}</article></main><Footer/></>
}
