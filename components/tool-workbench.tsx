...
export function ToolWorkbench({ kind }: Props) {
  ...
  return <Panel title="入力して結果を確認">
    ...
    {kind === 'width-converter' && <div className="mb-4 flex gap-2"><button type="button" className={widthMode === 'half' ? 'primary' : 'secondary'} onClick={() => update({ widthMode: 'half' })}>半角へ</button><button type="button" className={widthMode === 'full' ? 'primary' : 'secondary'} onClick={() => update({ widthMode: 'full' })}>全角へ</button></div>}
    {kind === 'base64' && <div className="mb-4 flex gap-2"><button type="button" className={mode === 'encode' ? 'primary' : 'secondary'} onClick={() => update({ mode: 'encode' })}>エンコード</button><button type="button" className={mode === 'decode' ? 'primary' : 'secondary'} onClick={() => update({ mode: 'decode' })}>デコード</button></div>}
...
function RouletteControls({ items, setItems }: { items: string[]; setItems: (v: string[]) => void }) {
  ...
  const [copied, setCopied] = useState(false)
  const spin = () => { const picked = items[Math.floor(Math.random() * items.length)]; if (!picked) return; if (picked !== result) setResult(picked); setHistory((h) => [picked, ...h.filter(x => x !== picked)].slice(0, 10)) }
  ...
  return <div className="grid gap-4"><div className="flex flex-wrap gap-2">{items.map((item, i) => <button type="button" className="pill" key={`${item}-${i}`} onClick={() => setItems(items.filter((_, j) => i !== j))}>{item} ×</button>)}</div><input className="input" aria-label="選択肢を追加" placeholder="選択肢を追加してEnter" onKeyDown={e => { if (e.key === 'Enter' && e.currentTarget.value.trim()) { setItems([...items, e.currentTarget.value.trim()]); e.currentTarget.value = '' } }} /><button type="button" className="primary" onClick={spin}>決める</button>{result && <div className="animate-pop-in rounded-2xl bg-primary/5 p-5 text-center"><p className="text-xs tracking-widest text-muted-foreground">結果</p><p className="mt-2 text-2xl font-bold text-primary">{result}</p><div className="mt-4 flex justify-center gap-3"><button type="button" className="secondary" onClick={spin}>もう一度</button><button type="button" className="secondary" onClick={copy}>{copied ? <><Check size={16}/>コピー済み</> : <><Copy size={16}/>結果をコピー</>}</button></div></div>}{history.length > 0 && <div className="rounded-2xl border border-border bg-card p-4"><p className="text-xs font-medium text-muted-foreground">履歴（直近10件）</p><ul className="mt-2 space-y-1 text-sm">{history.map((h, i) => <li key={`${h}-${i}`} className="text-muted-foreground">{i + 1}. {h}</li>)}</ul></div>}</div>
}
